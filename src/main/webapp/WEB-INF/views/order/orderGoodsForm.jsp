...
bookShop_pro
Public
Fork your own copy of hiareum/bookShop_pro
...
// 					jsf__pay() 함수를 호출
		...
		$.ajax({
			type : "post",
// 			async : false, //false인 경우 동기식으로 처리한다.
			url : "${contextPath}/test/naverPointOrder.do",
// 			url : "https://api.testpayup.co.kr/ep/api/kakao/himedia/order", (안됨)
			data : {
				"amount":amount
				,"itemName":itemName
				...
				<tbody>
					...
					<tr>
						<td>
							...
							value="카카오페이(간편결제)">카카오페이(간편결제) &nbsp;&nbsp;&nbsp; <input
							type="radio" id="pay_method" name="pay_method" value="네이버페이(카드)">네이버페이(카드)
							&nbsp;&nbsp;&nbsp; <input type="radio" id="pay_method"
			...
			<div id="popup_order_detail">
				...
				<form name="order_info" method="post" accept-charset="euc-kr">
					...
					<input type="hidden" name="naverpay_point_direct" value=""> 
						...
						<input type="hidden"
						name="module_type" value="01" /> <input type="hidden"
						name="ordr_chk" value="" /> <input type="hidden"
...
if (frm.res_cd.value == "0000") {
			...
			*/
			...
			//frm 서브밋하기
			//else if 네이버면
			else if(frm.naverpay_direct.value =="Y"){
		 	frm.method="post";
			frm.action="${contextPath}/test/naverPay.do";
			frm.submit(); 
			}
			
			else if(frm.naverpay_point_direct.value =="naverpay_point_direct"){
			 	frm.method="post";
				frm.action="${contextPath}/test/naverPay.do";
				frm.submit(); 
				}
